...
import com.sonycsl.echo.protocol.EchoLoraProtocol;
...
public final class EchoSocket {
    ...
    //Thang cmt: ID cua Gateway
    public static final String SELF_ADDRESS = "0.0.0.0";
    ...
    private static EchoLoraProtocol sLoraProtocol = new EchoLoraProtocol();

    private static Thread loraThread;
...
//        sTCPProtocol.openTCP();
        try {
            sLoraProtocol.openLora();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        startLoraReceiverThread();
    }

    private static void startLoraReceiverThread(){
        if (loraThread == null) {
            loraThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    sLoraProtocol.receive();
                }
            });
            loraThread.start();
        }
    ...
    public static void sendLoraFrame(EchoFrame frame) throws IOException {
        sLoraProtocol.sendLora(frame);
    }
