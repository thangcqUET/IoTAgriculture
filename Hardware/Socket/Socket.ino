...
void setup() {
//  pinMode(D1, OUTPUT); // dung cho o dien dung wemos
//  digitalWrite(D1, 1); // dung cho o dien dung wemos
  Serial.begin(115200);
  EEPROM.begin(512);
          ...
          switch(EPC){
            case OperationStatus:{
              ...
              for(int j=0;j<res.properties[i].PDC;j++){
                ...
                res.properties[i].EDT[j] = EEPROM.read(EEPROM_index_begin + j);
              }
              res.OPC++;
...
void sendEchoNode(IPAddress ipMulti, int portMulti ){
  ...
  byte echoMyNode_head[]={0x10, 0x81, 0x00, 0x00, 0x0E, 0xF0, 0x01, 0x0E, 0xF0, 0x01, 0x73, 0x02, 0xD5, PDC, qualityEOJs};
