...
//		if(data[1] != EHD2) return;
		...
		mSrcEchoInstanceCode = data[4];
		buffer = ByteBuffer.allocate(2);
		buffer.order(ByteOrder.BIG_ENDIAN);
		buffer.put(data[5]);
		buffer.put(data[6]);
		mDstEchoClassCode = buffer.getShort(0);
		mDstEchoInstanceCode = data[7];
		mESV = data[8];
		int size = data[9] & 0xFF;
		mPropertyList = new ArrayList<EchoProperty>();
		for(int i = 0, j = 10; i < size; i++) { //Thang cmt: j=12 -> j=10
			...
			if(pdc == 0) {
				...
				edt = new byte[(pdc & 0xFF)];
				System.arraycopy(data, j, edt, 0, (pdc & 0xFF));
				j += (pdc & 0xFF);
			}
